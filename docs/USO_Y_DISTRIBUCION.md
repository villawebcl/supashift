...
supashift profile add client-dev --account "dev@example.com"
```
...
### Abrir shell aislado
```bash
supashift shell client-a
```

### tmux por perfil
```bash
supashift tmux client-a
supashift tmux --many
```
...
```bash
supashift migrate-from-supabase-cli client-a --source auto --account "user1@example.com"
```
